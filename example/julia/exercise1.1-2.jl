...
# paramerters
q = [5000, 3000] # product fix cost
p = [100, 60] # cost per product
d = [400 400 800 800 1200 1200 1200 1200;
     200 200 200 200 200  200  500  500] # demand of product by month
s_init = [200,0] # initial stock
h = [5,3] # stock cost per product month
capacity = 1500
...
@constraint(model, max_cap[j in month], sum(x[i, j] for i in product) <= capacity)
# 目标函数
